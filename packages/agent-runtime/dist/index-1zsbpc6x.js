...
function buildA2AAgentCard(options = {}) {
  ...
  return {
    ...
    url,
    version: urVersion,
    documentationUrl: "https://github.com/Maitham16/ur-nexus/blob/master/docs/AGENT_TRENDS.md",
    capabilities: {
      streaming: true,
    ...
    securitySchemes: {
      bearer: {
        type: "http",
        scheme: "bearer",
        description: "Static shared-secret bearer token for task execution (ur a2a serve --token)."
      },
      delegation: {
        type: "http",
        scheme: "bearer",
        bearerFormat: "UR-Delegation",
        description: "Attenuated delegation token: scoped to specific skills, bound to this agent (aud), and time-limited. Mint with `ur a2a token mint` and verify with `ur a2a token verify`."
      }
    },
...
var urVersion, coverage, priorityRoadmap;
var init_trends = __esm(() => {
  urVersion = MACRO.VERSION;
  coverage = [
    {
      id: "local-runtime",
      ...
      professionalNextStep: "Let UR auto-apply the model-route recommendation per subagent step instead of only reporting it."
    },
    {
      id: "mcp",
    ...
    },
    {
      ...
      status: "covered",
      summary: "UR exposes a declarative DAG workflow format with approval/verification gates and resumable checkpoints, on top of session resume, background tasks, and task state.",
      evidence: [
    ...
    },
    {
      ...
      professionalNextStep: "Surface live per-iteration review verdicts and crew worker timelines in the agent-inspect view as they happen."
    },
    {
      id: "memory",
      name: "Long-term memory",
      status: "partial",
      summary: "UR has file-backed memory, research notes, team memory, forget controls, consolidation prompts, and a curated knowledge base with source-level provenance and retention; dense vector retrieval is still lexical rather than embedding-based.",
      evidence: [
    ...
    },
    {
      ...
      status: "partial",
      summary: "UR has verifier gates, project gates, /trace, a per-subagent run inspector, OpenTelemetry plumbing, release checks, and now a replayable eval harness with deterministic grading; published benchmark numbers and dashboards are the remaining layer.",
      evidence: [
      ...
      professionalNextStep: "Run the eval harness in CI and track pass-rate per category over time as a published number."
    },
    {
      id: "test-first-execution",
      name: "Test-first execution",
      status: "covered",
      summary: "UR can detect a project stack, run compile/test/lint commands as command evidence, store failed command traces, and install the detected command set into edit-time verifier gates.",
      evidence: [
      ...
      professionalNextStep: "Add per-package command plans for large monorepos with multiple quality stacks."
    },
    {
      id: "permission-safety",
      name: "Permission and safety policy",
      status: "covered",
      summary: "UR applies a project shell safety policy before broad command approvals: read/write/execute/network classes are separated, destructive commands require approval, risky operations are sandbox-recommended, and common secret exfiltration paths are denied.",
      evidence: [
      ...
      professionalNextStep: "Add OS-specific sandbox attestation to command evidence so every risky command records which sandbox was active."
    },
    {
      id: "context-management",
      name: "Project context management",
      status: "covered",
      summary: "UR can build a project manifest from repository manifests, instruction files, Project DNA, and verify/safety config; it also records decisions, constraints, commands, and diffs into durable task memory and writes compressed context summaries.",
      evidence: [
      ...
      professionalNextStep: "Feed the generated project manifest directly into subagent prompts and verifier gate selection."
    },
    {
      id: "security",
    ...
    },
    {
      ...
      status: "adapter-ready",
      summary: "UR has OAuth, XAA, MCP auth helpers, permissions, local trust boundaries, and now signed, attenuated, expiring delegation tokens for the A2A adapter — scoped to specific skills and bound to a single audience — so a holder can hand onward a strictly narrower capability without UR becoming an identity provider.",
      evidence: [
      ...
      professionalNextStep: "Federate delegation with a portable cross-agent identity only when UR exposes a network-facing A2A adapter beyond loopback."
    },
    {
      id: "multimodal",
      ...
      professionalNextStep: "Auto-pull or warn when a multimodal task has no capable local model, using the model-route gap signal."
    },
    {
      id: "scheduling",
      name: "Scheduled and recurring agents",
      status: "covered",
      summary: "UR turns cron-defined automations into ones that actually fire: a resident scheduler installs a launchd LaunchAgent (macOS), a systemd --user timer (Linux), or prints a crontab line, and an in-process daemon loop covers containers/CI.",
      evidence: [
      ...
      references: ["https://docs.langchain.com/oss/python/langgraph/overview"],
      professionalNextStep: "Add per-automation run history and failure alerting surfaced through /trace."
    },
    {
      id: "inbound-triggers",
      name: "Inbound chat/VCS triggers",
      status: "adapter-ready",
      summary: "UR parses a GitHub issue/PR comment or Slack mention webhook payload, decides whether a keyword (default /ur) should dispatch it, extracts the prompt, and can launch a headless run — the inbound counterpart to the bundled GitHub Action and install-slack-app/install-github-app helpers.",
      evidence: [
      ...
      professionalNextStep: "Ship a reference webhook receiver (serverless function) that calls ur trigger run with an actor allow-list."
    },
    {
      id: "sdk",
      name: "Programmatic SDK",
      status: "covered",
      summary: "A dependency-free TypeScript SDK (ur-agent/sdk: query, queryJSON, UrClient) plus a Python wrapper drive headless `ur -p`, inheriting the CLI permission model, MCP config, and local Ollama routing. The in-process counterpart to the loopback A2A server.",
      evidence: [
      ...
      professionalNextStep: "Publish the SDK as a documented subpath export and add a streaming (stream-json) iterator."
    }
  ];
  priorityRoadmap = [
...
export { buildAgentTrendReport, formatAgentTrendReport, formatA2AAgentCard, init_trends };
