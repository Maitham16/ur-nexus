...
async function createBridgeSession({
  ...
  const { getURAIOAuthTokens } = await import("./auth-kkft3e1e.js");
  const { getOrganizationUUID } = await import("./client-k7ttjc5s.js");
  const { getOauthConfig } = await import("./oauth-swm62xez.js");
  const { getOAuthHeaders } = await import("./api-p2ctwbng.js");
  const { parseGitHubRepository } = await import("./detectRepository-ggbvydz7.js");
  const { getDefaultBranch } = await import("./git-nzc4c7zx.js");
  const { getMainLoopModel } = await import("./model-jmf99dd1.js");
  const { default: axios } = await import("./index-ks0fhmg8.js");
  const accessToken = getAccessToken?.() ?? getURAIOAuthTokens()?.accessToken;
  if (!accessToken) {
  ...
  let gitOutcome = null;
  if (gitRepoUrl) {
    const { parseGitRemote } = await import("./detectRepository-ggbvydz7.js");
...
async function getBridgeSession(sessionId, opts) {
  ...
  const { getOAuthHeaders } = await import("./api-p2ctwbng.js");
  const { default: axios } = await import("./index-ks0fhmg8.js");
  const accessToken = opts?.getAccessToken?.() ?? getURAIOAuthTokens()?.accessToken;
  if (!accessToken) {
