...
Run: ur eval run ${suite2.name}`
  ...
  if (command === "report") {
    ...
    if (tokens.includes("--dashboard")) {
      const { buildDashboardHtml } = await import("./evals-xwcqxp3a.js");
...
Run it locally: ur eval run ${result.suite.name}`
  ...
  if (command === "run") {
    ...
    if (writeMetrics) {
      const { writeRunMetrics } = await import("./evals-xwcqxp3a.js");
...
` : "";
  ...
  if (command === "route") {
    ...
    const { listModelCapabilities } = await import("./model-doctor-8d7gegtz.js");
    const { resolveModelForTask } = await import("./modelRouter-424c9n2j.js");
    const { loadModelPool } = await import("./modelPool-gkvtnjcs.js");
