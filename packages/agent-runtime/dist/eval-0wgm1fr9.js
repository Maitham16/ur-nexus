...
Run: ur eval run ${suite2.name}`
  ...
  if (command === "report") {
    ...
    if (tokens.includes("--dashboard")) {
      const { buildDashboardHtml } = await import("./evals-b2wf27mr.js");
...
Run it locally: ur eval run ${result.suite.name}`
  ...
  if (command === "run") {
    ...
    if (writeMetrics) {
      const { writeRunMetrics } = await import("./evals-b2wf27mr.js");
...
` : "";
  ...
  if (command === "route") {
    ...
    const { listModelCapabilities } = await import("./model-doctor-an6jtygr.js");
    const { resolveModelForTask } = await import("./modelRouter-8snxzyt9.js");
    const { loadModelPool } = await import("./modelPool-xn61etwd.js");
