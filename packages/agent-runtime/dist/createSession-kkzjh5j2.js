...
async function createBridgeSession({
  ...
  const { getURAIOAuthTokens } = await import("./auth-ns2fz73t.js");
  const { getOrganizationUUID } = await import("./client-tq3hz3sz.js");
  const { getOauthConfig } = await import("./oauth-rqgdsh1a.js");
  const { getOAuthHeaders } = await import("./api-nn7a7kdd.js");
  const { parseGitHubRepository } = await import("./detectRepository-rnnz2ckv.js");
  const { getDefaultBranch } = await import("./git-dsbr44bh.js");
  const { getMainLoopModel } = await import("./model-hqx07qx5.js");
  const { default: axios } = await import("./index-z2yqpdx4.js");
  ...
  let gitOutcome = null;
  if (gitRepoUrl) {
    const { parseGitRemote } = await import("./detectRepository-rnnz2ckv.js");
