...
async function createBridgeSession({
  ...
  const { getURAIOAuthTokens } = await import("./auth-8pdkt1cf.js");
  const { getOrganizationUUID } = await import("./client-4x978n50.js");
  const { getOauthConfig } = await import("./oauth-cp15d8ap.js");
  const { getOAuthHeaders } = await import("./api-5b5m9txp.js");
  const { parseGitHubRepository } = await import("./detectRepository-k2e0dsm6.js");
  const { getDefaultBranch } = await import("./git-8xspjqqb.js");
  const { getMainLoopModel } = await import("./model-yxjak6qe.js");
  ...
  let gitOutcome = null;
  if (gitRepoUrl) {
    const { parseGitRemote } = await import("./detectRepository-k2e0dsm6.js");
