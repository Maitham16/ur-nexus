...
async function loadVerifyConfig(cwd) {
  ...
  try {
    ...
    if (Array.isArray(cfg.afterEdit)) {
      out.afterEdit = cfg.afterEdit.filter((c) => typeof c === "string");
    }
    if (Array.isArray(cfg.afterBash)) {
      out.afterBash = cfg.afterBash.filter((c) => typeof c === "string");
    }
    if (Array.isArray(cfg.ignorePatterns)) {
      out.ignorePatterns = cfg.ignorePatterns.filter((p) => typeof p === "string");
    }
...
function pickCommands(config, modifiedFiles, ranBash, cwd) {
  ...
  if (ranBash && filteredEdits.length === 0 && config.afterBash && config.afterBash.length > 0) {
    return config.afterBash;
  }
  return null;
}
function hasNonIgnoredEdits(config, modifiedFiles, cwd) {
  return filterIgnored(modifiedFiles, config?.ignorePatterns, cwd).length > 0;
}
function filterIgnored(files, ignorePatterns, cwd) {
  if (!ignorePatterns || ignorePatterns.length === 0)
    return files;
  const matcher = import_picomatch.default(ignorePatterns);
  return files.filter((file) => {
    const candidates = pathCandidates(file, cwd);
    return !candidates.some((candidate) => matcher(candidate));
  });
}
function pathCandidates(file, cwd) {
...
async function runGateCommands(commands, cwd, timeoutMs = DEFAULT_TIMEOUT_MS) {
  let ran = 0;
...
function runSingleCommand(command, cwd, timeoutMs) {
...
function buildFailureReminder(command, result) {
  const parts = [
...
${stderrTrimmed}`);
  ...
  parts.push(`You may not declare the task complete until this command exits 0. ` + `Read the output, fix the underlying issue, and re-run the work this ` + `turn — do not just retry the verify command.`);
  return parts.join(`

`);
}
var import_picomatch, DEFAULT_TIMEOUT_MS = 60000, MAX_OUTPUT_CHARS_PER_STREAM = 4000;
var init_projectGates = __esm(() => {
  import_picomatch = __toESM(require_picomatch2(), 1);
});

export { require_picomatch2 as require_picomatch, loadVerifyConfig, pickCommands, hasNonIgnoredEdits, runGateCommands, init_projectGates };
