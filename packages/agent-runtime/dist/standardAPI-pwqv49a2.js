...
async function createStandardAPIClient(options) {
  ...
  async function doRequest(params, requestOptions) {
    ...
    const response = await axiosPostWithProviderReliability(endpoint, buildAPIRequest(family, params), {
      headers: {
  ...
  async function doStream(params, requestOptions, controller) {
    ...
    const clientRequestId = params?.headers?.["x-client-request-id"];
    const response = await axiosPostWithProviderReliability(endpoint, buildAPIRequest(family, { ...params, stream: true }), {
      headers: {
        "Content-Type": "application/json",
        ...buildAuthHeaders(family, apiKey, params),
        ...clientRequestId && { "x-client-request-id": clientRequestId },
    ...
    const data = family === "anthropic" ? createAnthropicSSEMessageStream(response.data, streamOptions) : family === "google" ? createGeminiSSEMessageStream(response.data, streamOptions) : createOpenAISSEMessageStream(response.data, streamOptions);
...
function getAPIEndpoint(family, baseUrl, model, stream) {
  switch (family) {
    case "openai":
      return normalizeProviderEndpoint(baseUrl, "https://api.openai.com/v1", "/chat/completions");
    case "anthropic":
      return normalizeProviderEndpoint(baseUrl, "https://api.anthropic.com/v1", "/messages");
    case "google": {
  ...
  switch (family) {
    ...
    case "google": {
      ...
      return {
        ...
        ...tools.length > 0 ? { tools } : {},
        ...params.tool_choice !== undefined ? { toolConfig: toGeminiToolConfig(params.tool_choice) } : {},
        generationConfig: {
  ...
  switch (family) {
    ...
    case "anthropic": {
      ...
      if (data.stop_reason === "tool_use" && !hasToolUse(anthropicContent)) {
        throw new ProviderResponseParseError("anthropic response stopped for tool_use but did not include a tool_use block", { data });
      }
      return {
    ...
    case "google": {
      ...
      if (data.candidates?.[0]?.finishReason === "FUNCTION_CALL" && !hasToolUse(content)) {
        throw new ProviderResponseParseError("gemini response finished with FUNCTION_CALL but did not include a functionCall part", { data });
      }
      return {
        ...
        model: fallbackModel,
        content,
        stop_reason: mapStopReason(data.candidates?.[0]?.finishReason, hasToolUse(content)),
        stop_sequence: null,
        usage: {
...
function mapStopReason(reason, includesToolUse = false) {
  if (includesToolUse || reason === "tool_calls" || reason === "function_call" || reason === "tool_use" || reason === "FUNCTION_CALL") {
    return "tool_use";
  }
  switch (reason) {
    case "length":
...
function toAnthropicMessages(messages) {
  ...
  return messages.map((message, messageIndex) => ({
    ...message,
    content: toAnthropicContent(message?.content, `messages[${messageIndex}].content`)
  }));
}
function toAnthropicContent(content, context) {
  if (typeof content === "string")
    return content;
  if (!Array.isArray(content))
    return "";
  return content.map((block, index) => toAnthropicContentBlock(block, `${context}[${index}]`));
}
function toAnthropicContentBlock(block, context) {
  ...
  if (block?.type === "tool_result") {
    return {
      ...block,
      content: toAnthropicToolResultContent(block.content, `${context}.content`)
    };
  }
  return block;
}
function toAnthropicToolResultContent(content, context) {
  if (typeof content === "string")
    return content;
  if (!Array.isArray(content))
    return content;
  return content.map((block, index) => toAnthropicToolResultBlock(block, `${context}[${index}]`));
}
function toAnthropicToolResultBlock(block, context) {
  if (typeof block === "string")
    return { type: "text", text: block };
  if (block?.type === "text") {
    return { type: "text", text: block.text ?? "" };
...
function toAnthropicTools(tools) {
  if (!Array.isArray(tools))
    return [];
  return tools.filter((tool) => typeof tool?.name === "string" && ("input_schema" in tool)).map((tool) => ({
    name: tool.name,
  ...
  const result = content.map((block, index) => {
    ...
    if (block?.type === "tool_use") {
      if (typeof block.id !== "string" || block.id.length === 0 || typeof block.name !== "string" || block.name.length === 0) {
        throw new ProviderResponseParseError(`anthropic content[${index}] is an invalid tool_use block`, { block });
      }
      return {
        type: "tool_use",
        id: block.id,
...
function parseGeminiParts(parts) {
  ...
  for (const [index, part] of parts.entries()) {
    ...
    if (typeof call?.name !== "string" || call.name.length === 0) {
      throw new ProviderResponseParseError(`gemini candidates[0].content.parts[${index}].functionCall.name is required`, { functionCall: call });
    }
    content.push({
      type: "tool_use",
      id: typeof call.id === "string" && call.id.length > 0 ? call.id : `gemini_tool_${randomUUID()}`,
      name: call.name,
      input: parseGeminiFunctionArgs(call.args, `gemini candidates[0].content.parts[${index}].functionCall.args`)
    });
  }
  return content.length > 0 ? content : [{ type: "text", text: "" }];
}
function parseGeminiFunctionArgs(args, path) {
...
function contentToGeminiParts(content, toolNamesById) {
  if (typeof content === "string")
    return [{ text: content }];
  if (!Array.isArray(content))
    return [{ text: "" }];
...
`);
  ...
  for (const [index, block] of content.entries()) {
    ...
    switch (block?.type) {
      ...
      case "tool_result":
        flushTextPart();
        assertNoImageBlocks(block.content, "gemini", `tool_result ${block.tool_use_id ?? index} content`);
        parts.push({
          functionResponse: {
...
function imageBlockToGeminiPart(block, context) {
  ...
  if (!source.mediaType) {
    throw new ProviderCapabilityError("gemini image URL content requires media_type for fileData mapping", { providerName: "gemini", capability: "multimodal_input", context });
  }
  return {
    fileData: {
      mimeType: source.mediaType,
