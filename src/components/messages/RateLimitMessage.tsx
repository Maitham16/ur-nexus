...
import { getRateLimitTier, getSubscriptionType, isURAISubscriber } from 'src/utils/auth.js';
...
export function RateLimitMessage(t0) {
  ...
  const isCurrentlyRateLimited = urAiLimits.status === "rejected" && urAiLimits.resetsAt !== undefined && !urAiLimits.isUsingOverage;
  ...
  bb0: {
    ...
    if ($[7] !== shouldAutoOpenRateLimitOptionsMenu) {
      t7 = getUpsellMessage({
        ...
        isTeamOrEnterprise,
        hasBillingAccess: hasURAiBillingAccess()
      });
