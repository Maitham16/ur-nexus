...
const getRipgrepConfig = memoize((): RipgrepConfig => {
  ...
  if (!existsSync(command)) {
    logForDebugging(
      `[ripgrep] Builtin ripgrep not found at ${command}; falling back to system rg`,
    )
    return { mode: 'system', command: 'rg', args: [] }
  }
...
function ripGrepRaw(
  ...
  const parsedSeconds =
    parseInt(process.env.UR_CODE_GLOB_TIMEOUT_SECONDS || '', 10) || 0
...
export async function ripGrep(
  ...
  return new Promise((resolve, reject) => {
    const handleResult = (
      ...
      // If we timed out with no results, throw an error so UR knows the search
