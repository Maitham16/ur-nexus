...
const resolveCanonicalRoot = memoizeWithLRU(
  (gitRoot: string): string => {
    try {
      ...
      // dialog and executing hooks from .ur/settings.json on startup.
      //
      ...
      // Use the common dir itself as the stable identity (urhqs/ur#27994).
...
export const stashToCleanState = async (message?: string): Promise<boolean> => {
  try {
    const stashMessage =
      message || `UR auto-stash - ${new Date().toISOString()}`
