...
export const getGlobalURFile = memoize((): string => {
  // Legacy fallback for backwards compatibility
  if (
    getFsImplementation().existsSync(
      join(getURConfigHomeDir(), '.config.json'),
    )
  ) {
    return join(getURConfigHomeDir(), '.config.json')
  }

  const filename = `.ur${fileSuffixForOauthConfig()}.json`
  return join(process.env.UR_CONFIG_DIR || homedir(), filename)
})
 ...
 * If UR_CODE_HOST_PLATFORM is set to a valid platform value, that overrides
