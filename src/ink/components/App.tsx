...
export default class App extends PureComponent<Props, State> {
  ...
  // Parsed stdin items waiting for ordered delivery. If text and Return
  // arrive in the same raw chunk, Return is deferred one tick so controlled
  // inputs can commit the text before their submit handler reads it.
  pendingInputItems: ParsedInput[] = [];
  pendingInputDrain: ReturnType<typeof setImmediate> | null = null;
  ...
  override componentWillUnmount() {
    ...
    if (this.pendingInputDrain) {
      clearImmediate(this.pendingInputDrain);
      this.pendingInputDrain = null;
    }
  ...
  processInput = (input: string | Buffer | null): void => {
    ...
    if (keys.length > 0) {
      this.pendingInputItems.push(...keys);
      this.drainPendingInputItems();
    }
  ...
  schedulePendingInputDrain = (): void => {
    if (this.pendingInputDrain) {
      return;
    }
    this.pendingInputDrain = setImmediate(() => {
      this.pendingInputDrain = null;
      this.drainPendingInputItems();
    });
  };
  drainPendingInputItems = (): void => {
    if (this.pendingInputDrain) {
      return;
    }
    while (this.pendingInputItems.length > 0) {
      const returnIndex = this.pendingInputItems.findIndex(item => item.kind === 'key' && item.name === 'return');
      if (returnIndex === -1) {
        processInputItems(this, this.pendingInputItems.splice(0));
        return;
      }
      if (returnIndex > 0) {
        processInputItems(this, this.pendingInputItems.splice(0, returnIndex));
        flushReconcilerWork();
        this.schedulePendingInputDrain();
        return;
      }
      processInputItems(this, this.pendingInputItems.splice(0, 1));
      flushReconcilerWork();
    }
  };
  handleReadable = (): void => {
  ...
  handleSuspend = (): void => {
    ...
    const resumeHandler = () => {
      ...
      // Emit resume event for UR to handle
...
function processInputItems(app: App, items: ParsedInput[]): void {
  if (items.length === 0) {
    return;
  }
  // Process each contiguous group in one discreteUpdates call to prevent
  // "Maximum update depth exceeded" when many keys arrive at once (paste or
  // key repeat), while allowing explicit boundaries before Return.
  reconciler.discreteUpdates(processKeysInBatch, app, items, undefined, undefined);
}

function flushReconcilerWork(): void {
