 ...
 *   1. UR_CODE_DISABLE_AUTO_MEMORY env var (1/true → OFF, 0/false → ON)
 *   2. UR_CODE_SIMPLE (--bare) → OFF
 *   3. CCR without persistent storage → OFF (no UR_CODE_REMOTE_MEMORY_DIR)
...
export function isAutoMemoryEnabled(): boolean {
  ...
  if (
    ...
    !process.env.UR_CODE_REMOTE_MEMORY_DIR
  ) {
    return false
  }
 ...
 * Resolution order:
 *   1. UR_CODE_REMOTE_MEMORY_DIR env var (explicit override, set in CCR)
 *   2. ~/.ur (default config home)
 */
...
function getAutoMemPathOverride(): string | undefined {
  return validateMemoryPath(
    process.env.UR_COWORK_MEMORY_PATH_OVERRIDE,
    false,
  )
}
 ...
 *
 * SECURITY: projectSettings (.ur/settings.json committed to the repo) is
...
/**
 * Check if UR_COWORK_MEMORY_PATH_OVERRIDE is set to a valid override.
 ...
 * same repo share one auto-memory directory (urhqs/ur#24382).
 */
 ...
 *
 * Resolution order:
 *   1. UR_COWORK_MEMORY_PATH_OVERRIDE env var (full-path override, used by Cowork)
 ...
 * env vars / settings.json / UR_CONFIG_DIR are session-stable in
 ...
 *
 * When UR_COWORK_MEMORY_PATH_OVERRIDE is set, this matches against the
