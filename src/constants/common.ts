...
export function getLocalISODate(): string {
  ...
  if (process.env.UR_CODE_OVERRIDE_DATE) {
    return process.env.UR_CODE_OVERRIDE_DATE
  }
...
export function getLocalMonthYear(): string {
  const date = process.env.UR_CODE_OVERRIDE_DATE
    ? new Date(process.env.UR_CODE_OVERRIDE_DATE)
    : new Date()
