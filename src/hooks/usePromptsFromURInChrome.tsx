...
import { UR_IN_CHROME_MCP_SERVER_NAME, isTrackedURInChromeTabId } from '../utils/urInChrome/common.js';
import { lazySchema } from '../utils/lazySchema.js';
import { enqueuePendingNotification } from '../utils/messageQueueManager.js';

// Schema for the prompt notification from Chrome extension (JSON-RPC 2.0 format)
const URInChromePromptNotificationSchema = lazySchema(() => z.object({
...
/**
 * A hook that listens for prompt notifications from the UR for Chrome extension,
 * enqueues them as user prompts, and syncs permission mode changes to the extension.
 */
export function usePromptsFromURInChrome(mcpClients, toolPermissionMode) {
  const $ = _c(6);
  useRef(undefined);
  let t0;
  if ($[0] !== mcpClients) {
...
function findChromeClient(clients: MCPServerConnection[]): ConnectedMCPServer | undefined {
  return clients.find((client): client is ConnectedMCPServer => client.type === 'connected' && client.name === UR_IN_CHROME_MCP_SERVER_NAME);
}
