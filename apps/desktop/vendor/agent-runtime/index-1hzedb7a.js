...
// src/services/agents/ciLoop.ts
function buildFixPrompt(command, summary, allowGenerated = false) {
...
function detectPublicApiChanges(filePaths) {
...
function detectDeletionIntent(output) {
  const patterns = [
    /\brm\s+(?:-[rf]+\s+)?\S+/i,
    /\brmdir\b/i,
    /\brimraf\b/i,
...
function summarizeFailure(output, maxLines = 40) {
  const lines = output.split(`
`);
  const flagged = lines.filter((line) => /\b(error|fail(ed|ure)?|exception|expected|assert|✗|✖|×|cannot|not found|undefined)\b/i.test(line));
...
async function runCiLoop(options) {
  ...
  const { file, args } = options.execTarget ? wrapCommand(options.execTarget, parsed, cwd) : parsed;
  const attempts = [];
  const constitutionWarnings = new Set;
  let lastFailure;
  const addConstitutionWarning = (warning) => {
    constitutionWarnings.add(warning);
  };
  const withConstitutionWarnings = (result) => {
    const warnings = [
...
`));
      appendRunAction(cwd, runId, {
        ...
        nextAction: run.code === 0 ? "record success and finish" : "summarize failure and invoke fix agent if attempts remain",
        data: { attempt }
      });
      if (run.code === 0) {
        attempts.push({ attempt, code: 0, passed: true });
        if (lastFailure) {
          recordResolution(cwd, command, `Command passed on attempt ${attempt} after ${attempt - 1} failed attempt(s).`);
        }
...
${run.stderr}`);
    ...
    if (requireApprovalForDeletion) {
      ...
      const deletedFiles = changedFileDetails.filter((change) => change.deleted).map((change) => change.path);
    ...
    if (!allowGenerated) {
      ...
      if (generatedChanged.length > 0) {
        ...
        const restoreNote = restore.code === 0 ? `Generated/vendor files were restored after violation: ${generatedChanged.join(", ")}` : `Generated/vendor files changed and automatic restore failed: ${generatedChanged.join(", ")}`;
    ...
    if (options.commit || options.push) {
      ...
      if (!allowGenerated) {
        ...
        if (publicApiChanged.length > 0) {
          last.blockedByReview = true;
          addConstitutionWarning(`Public API surface changed by fix; commit/push blocked until reviewed: ${publicApiChanged.join(", ")}`);
          return finish(withConstitutionWarnings({
            command,
            status: "blocked",
            attempts
  ...
  const cannotFixReason = finalAttempt?.constitutionNotes ? `Constitution violations prevented fixes: ${finalAttempt.constitutionNotes.join("; ")}` : `The command "${command}" still failed after ${maxAttempts} attempts. The fix agent could not make it pass.`;
...
var AGENT_CONSTITUTION = `AGENT CONSTITUTION (hard rules — violation is a bug):
...
5. NEVER CHANGE PUBLIC API (exported names, function signatures, CLI flags, env vars) without an explicit warning and, when possible, backwards compatibility. Prefer additive changes.`, defaultExec = async (file, args, cwd) => {
...
export { splitCommand, summarizeFailure, runCiLoop, formatCiLoopResult, init_ciLoop };
