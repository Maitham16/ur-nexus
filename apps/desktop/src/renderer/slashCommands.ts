export type DesktopSlashCommand = {
  ...
  description: string
  argumentHint?: string
  category: 'Workspace' | 'Agent' | 'Context' | 'Developer'
  action: 'local' | 'navigate' | 'prompt'
  route?: string
}

// Desktop-safe counterparts for the terminal commands people use most. The
// palette deliberately includes only commands with meaningful desktop
// behavior; unsupported terminal-only TUI dialogs are not advertised.
export const DESKTOP_SLASH_COMMANDS: readonly DesktopSlashCommand[] = [
  ...
  { name: 'review', description: 'Review code for correctness and maintainability', argumentHint: '[scope]', category: 'Agent', action: 'prompt' },
  { name: 'security-review', description: 'Run a focused security review', argumentHint: '[scope]', category: 'Agent', action: 'prompt' },
  { name: 'test-first', description: 'Implement with a test-first verification loop', argumentHint: '<task>', category: 'Agent', action: 'prompt' },
  { name: 'commit', description: 'Prepare a verified Git commit', argumentHint: '[message]', category: 'Developer', action: 'prompt' },
  { name: 'context', description: 'Analyze the active project context', category: 'Context', action: 'prompt' },
  { name: 'memory', description: 'Summarize relevant project memory', argumentHint: '[topic]', category: 'Context', action: 'prompt' },
  { name: 'remember', description: 'Remember a durable project fact', argumentHint: '<fact>', category: 'Context', action: 'prompt' },
  { name: 'doctor', description: 'Diagnose project and agent configuration', category: 'Developer', action: 'prompt' },
  { name: 'worktree', description: 'Inspect or create an isolated worktree', argumentHint: '[task]', category: 'Developer', action: 'prompt' },
] as const
